...
def main():
    ...
    LOCAL_IMAGE_PATH = r"C:\Users\22754\OneDrive\Desktop\G1_workspace\G1_output\color\g1_images.jpg"  # 需要上传的图片路径
    print(f"检查文件路径: {LOCAL_IMAGE_PATH}")
    print(f"文件是否存在: {os.path.exists(LOCAL_IMAGE_PATH)}")
    if not os.path.exists(LOCAL_IMAGE_PATH):
        print("图片文件未找到！")
        return
    else:
        print(f"文件大小: {os.path.getsize(LOCAL_IMAGE_PATH)} bytes")
    
    LOCAL_RESULT_PATH = r"C:\Users\22754\OneDrive\Desktop\G1_workspace\G1_output\g1_poses.json"  # 本地保存处理后结果的路径
                    ...
                    if transfer.file_exists(REMOTE_RESULT_PATH):
                        ...
                        if current_mtime is not None and current_mtime != last_result_mtime:
                            ...
                            if transfer.download_result(REMOTE_RESULT_PATH, LOCAL_RESULT_PATH):
                                ...
                                print("download failed, keep the last modification time")
